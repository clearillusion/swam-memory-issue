...
folder are mapped to the application's root namespace, 'CrashIssue'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "CrashIssue"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'CrashIssue' in the same way
as `"CrashIssue/sass/src"`.
